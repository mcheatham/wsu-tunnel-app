...
exports.getPath = async function(startID, endID) {
    const nodes = await database.allNodes();
    const edges = await database.allConnections();
    ...
    while(toVisit.length > 0) {
        ...
        let incidentEdgeIDs = await database.incidentEdges(thisNodeID);
    ...
    while(path.nodeIDs[path.nodeIDs.length - 1] !== startID) {
        ...
        // eslint-disable-next-line
        let incidentEdgeIDs = await database.incidentEdges(thisID);
