...
app.get('/getPath/:start-:end', async (req, res) => {
    ...
    const edges = await query('SELECT * FROM connections');
    ...
    while(toVisit.length > 0) {
        ...
        let incidentEdgeIDs = await query('SELECT connectionID FROM connections WHERE nodeA_ID=? OR nodeB_ID=?', [thisNodeID, thisNodeID]);
    ...
    while(path.nodeIDs[path.nodeIDs.length - 1] !== startID) {
        ...
        // eslint-disable-next-line
        let incidentEdgeIDs = await query('SELECT connectionID FROM connections WHERE nodeA_ID=? OR nodeB_ID=?', [thisID, thisID]);
